...
async def test_paginator_interface_reactions():
    ...
    with utils.mock_ctx(bot) as ctx:
        ...
        emoji = nextcord.PartialEmoji(
            animated=False, name="\N{BLACK RIGHT-POINTING TRIANGLE}", id=None
        )
        bot.dispatch(
            "raw_reaction_add",
            nextcord.RawReactionActionEvent(payload, emoji, "REACTION_ADD")
            if nextcord.version_info >= (1, 3)
            else nextcord.RawReactionActionEvent(payload, emoji),
        )
        ...
        # push left button
        emoji = nextcord.PartialEmoji(
            animated=False, name="\N{BLACK LEFT-POINTING TRIANGLE}", id=None
        )
        bot.dispatch(
            "raw_reaction_add",
            nextcord.RawReactionActionEvent(payload, emoji, "REACTION_ADD")
        ...
        # push close button
        emoji = nextcord.PartialEmoji(
            animated=False, name="\N{BLACK SQUARE FOR STOP}", id=None
        )
        bot.dispatch(
            "raw_reaction_add",
            nextcord.RawReactionActionEvent(payload, emoji, "REACTION_ADD")
