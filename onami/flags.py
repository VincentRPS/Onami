...
The onami cog base, which contains most of the actual functionality of onami.
...
class Flag:
    """
    Dataclass that represents a onami flag state. Only for internal use.
    """
    ...
    def resolve(self, flags):  # pylint: disable=too-many-return-statements
        ...
        env_value = os.getenv(f"onami_{self.name}", "").strip()
...
class Flags(metaclass=FlagMeta):  # pylint: disable=too-few-public-methods
    ...
    Or you can override them programmatically:
        onami.Flags.HIDE = True
    """

    # Flag to indicate the onami base command group should be hidden
    HIDE: bool
