...
"""
onami.features.python
~~~~~~~~~~~~~~~~~~~~~~~~

The onami Python evaluation/execution commands.
...
from onami.codeblocks import codeblock_converter
...
from onami.functools import AsyncSender
from onami.paginators import PaginatorInterface, WrappedPaginator, use_file_check
from onami.repl import AsyncCodeExecutor, Scope, all_inspections, disassemble, get_var_dict_from_ctx
...
class PythonFeature(Feature):
    ...
    @Feature.Command(parent="oni", name="retain")
    async def oni_retain(self, ctx: commands.Context, *, toggle: bool = None):
        """
    ...
    @Feature.Command(parent="oni", name="py", aliases=["python"])
    async def oni_python(self, ctx: commands.Context, *, argument: codeblock_converter):
        """
            ...
            async with ReplResponseReactor(ctx.message):
                with self.submit(ctx):
                    ...
                    async for send, result in AsyncSender(executor):
                        ...
                        send(await self.oni_python_result_handling(ctx, result))
